...
async function initVocabulary(){
 ...
 document.querySelectorAll('[data-vocab-practice]').forEach(btn=>btn.onclick=()=>{const item=rows.find(x=>x.id===btn.dataset.vocabPractice);if(!item)return;const value=prompt(item.learning.state==='seen'?`Что это? ${item.visual}`:`Скажи по-бурятски: ${item.ru}`)||'';if(!value.trim())return;const ok=answerMatches(value.trim(),item.bxr);if(ok){const s=stateFor(item.id);s.seen=Number(s.seen||0)+1;if(s.state==='seen'){s.recognized=Number(s.recognized||0)+1;s.state=s.recognized>=2?'recognized':'seen';}else{s.active=Number(s.active||0)+1;s.state=s.active>=3?'automatic':'active';}localStorage.setItem(`buryad.vocab.${item.id}`,JSON.stringify(s));document.dispatchEvent(new CustomEvent('buryad:vocabulary-change'));document.dispatchEvent(new CustomEvent('buryad:daily-complete',{detail:{stage:'words'}}));}else{alert(`Опора: ${item.bxr}`);}});
}
